...
3. 账号更安全，登录采用 **JWT** 签发会话令牌 ~~词元（token）~~，支持多设备管理，可随时踢出陌生设备。修改密码、被封禁时全部令牌立即失效。
4. 全平台，tfc 基于 Flutter 构建，支持 Windows、macOS、Linux、Android，目前只差 iOS 由于~~某科技大厂~~原因无法上线。
5. 功能丰富，论坛、私聊、群聊、公告、表情包、文件共享应有尽有，还支持好友之间的 **P2P 语音视频通话**（媒体流点对点传输，不经过服务器）。更多功能等待你的探索。
6. 文件传输高效，支持 **SHA-256 秒传**（服务器已有该文件时直接登记，不再上传）与大文件**分块上传**（断点续传 + 完整性校验）。
7. 轻量级，客户端安装包大部分控制在 70MB 以内，MacOS 由于特殊原因也控制在 200MB 以内。Server 也是很轻量的，极速部署。
8. 双端开源，放心使用，不惧下毒。
9. 聊天界面支持 Markdown、LaTeX，设计圆润，符合 OIer 审美（逃）。
...
你需要有 `python3`（建议 3.10+）与 `git`。这里以 ubuntu 为例，其余大同小异。
...
> - **MySQL / PostgreSQL 数据库**：当前为实验性支持，请谨慎使用（如果可以的话最好还是使用 SQLite）（目前问题很多呀）。
> - **OSS2（阿里云对象存储）**：将文件存储转移到云端，节省本地磁盘。
> - **反向代理**：使用 Nginx/Caddy 时开启，并正确设置代理层数。
> - **RTC / TURN**：客户端之间可进行 P2P 语音视频通话，若用户处于严格 NAT 环境，需自行部署 TURN 并写入 `rtc` 配置。
...
- **聊天**：私聊、群聊、群聊精华、消息置顶与引用、@ 提及、消息转发与撤回、Markdown / LaTeX 渲染、全局消息搜索
- **通话**：与好友进行 P2P 语音视频通话，支持静音、开关摄像头、切换摄像头
- **文件**：拖拽发送图片、视频、音频与任意文件，支持在线预览与多媒体播放；大文件分块上传、秒传、离线缓存
- **表情包**：内置表情包市场，支持创建自己的表情包并分享
- **论坛**：发帖、评论、回复，支持富文本与附件
- **账号**：自定义头像、个性签名、个人简介，多设备会话管理，通知免打扰设置
- **安全**：主密码 / 生物识别解锁、RSA 公钥校验、外部链接与图片保护
- **数据**：聊天记录本地存储与离线同步，支持导出聊天记录、备份本地数据库
- **连接**：内置连通性自检、媒体代理与多开支持，公网 / 内网 / 反向代理环境均可适配
